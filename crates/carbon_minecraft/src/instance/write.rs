...
use crate::instance::write::InstanceWriteError::{
    MinecraftPackageAlreadyExist, MinecraftPackageIsAFile,
};
...
pub enum InstanceWriteError {
    ...
    InstanceConfigurationWritingError(
        #[from] instance::configuration::ConfigurationFileParsingError,
    ),
...
pub async fn write_at<T: AsRef<Path> + Sync>(instance: Instance, path: &T) -> InstanceWriteResult {
    ...
    let minecraft_package_folder =
        &PathBuf::from(base_path).join(instance::consts::MINECRAFT_PACKAGE_RELATIVE_PATH);
    ...
    match (
        ...
        (InstanceStatus::Persisted(instance_path), false, false)
    ...
    let instance_configuration_file_path =
        PathBuf::from(base_path).join(instance::consts::CONFIGURATION_FILE_RELATIVE_PATH);
