...
fn main() {
    ...
    if cfg!(test) {
        let test_build_output_path = env!("CARGO_TARGET_TMPDIR", "unable to get CARGO_TARGET_DIR !!!");
        ...
        std::fs::remove_dir(destination_test_assets_folder)?;
        std::fs::copy(origin_test_assets_folder, destination_test_assets_folder)?;
