...
impl InstanceManager {
    ...
    pub async fn get_all_instances(&self) -> BTreeSet<Instance> {
        debug!("trying to retrieve all instances");
    ...
    pub async fn get_instance_by_id(&self, id: u128) -> Result<Instance, InstanceManagerError> {
        debug!("trying to retrieve instance with id {id}");
        self.instance_store
    ...
    pub async fn add_instance(
        ...
        debug!(
            "trying to add instance with dto {}",
            serde_json::to_string(&dto).unwrap_or("<<unrepresentable dto>>".into())
        );
    ...
    pub async fn delete_instance_by_id(
        ...
        debug!(
            "trying to remove instance with id {id} put in trash bin {}",
            !remove_from_fs
        );
        ...
        Ok(instance_to_delete.clone())
    }

    pub async fn _read_instances_from_directory(
        &self,
    ...
    pub async fn patch_instance_by_id(
        ...
        debug!("trying to patch instance with id {id} with new values {new_values:#?}");
        ...
        for forbidden_property_key in forbidden_property_keys {
            if let Some(value) = new_values_cloned.remove(forbidden_property_key) {
                warn!("removing property named {forbidden_property_key} from patch plan cause is forbidden to change with patch, value proposed in patch plan {value:?}")
            }
        }
    ...
    async fn change_instance_persistence_status(
        ...
        let written_instance = match new_persistence_status {
            ...
            new_status => Err(InstanceManagerError::InstanceStatusPatchingNotAllowed(
                new_status,
            ))?,
        };
