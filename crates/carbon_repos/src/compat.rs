...
/// The fresh-install baseline (spec §11): the committed, normalized schema
...
/// dump (spec §11: it is the same normalized `sqlite_master` artifact the
...
impl MigrationSet {
    ...
    ///
    /// - Truly fresh install (empty `sqlite_master`, spec §11): execute the
    ...
    pub fn open(&self, conn: &mut Connection, db_path: &Path) -> DbResult<OpenVerdict> {
        ...
        // chain can be replaced by one vetted baseline (spec §11). This check
    ...
    /// Fresh-install path (spec §11): executes the committed baseline in one
    ...
    fn install_baseline(&self, conn: &mut Connection) -> DbResult<OpenVerdict> {
        ...
        // starts one (matching `to_latest`/`open`, spec §7.1).
        conn.pragma_update(None, "foreign_keys", "OFF")?;
        let tx = conn.transaction()?;
