...
fn derives_name_based_from_row_with_columns_metadata() {
    ...
    let got: Sample = conn
        .query_row("SELECT id, fullVersion, major, isValid, type, tokenExpires, blob FROM s", [],
                   Sample::from_row)
        .unwrap();
    ...
    assert_eq!(got.token_expires, Some(from_millis(1_784_557_692_104).unwrap()));
    ...
    assert_eq!(names, vec!["id", "fullVersion", "major", "isValid", "type", "tokenExpires", "blob"]);
    let te = Sample::COLUMNS.iter().find(|c| c.name == "tokenExpires").unwrap();
    assert!(te.nullable);
