...
fn version_info_upsert_inserts_then_updates_payload_and_freshness() {
    ...
    assert_eq!(
        vm::upsert_version_info(&conn, "1.20.1", b"payload-v1", t1).unwrap(),
        1
    );
    let row = vm::get_version_info(&conn, "1.20.1").unwrap().unwrap();
    ...
    vm::upsert_version_info(&conn, "1.20.1", b"payload-v2", t2).unwrap();
    let row = vm::get_version_info(&conn, "1.20.1").unwrap().unwrap();
...
fn version_info_get_missing_returns_none() {
    let (_d, conn) = migrated_db();
    assert!(vm::get_version_info(&conn, "nope").unwrap().is_none());
}
...
fn partial_version_info_upsert_inserts_then_updates_payload_and_freshness() {
    ...
    let t1 = DbDateTime(from_millis(1_700_000_000_000).unwrap());
    assert_eq!(
        vm::upsert_partial_version_info(&conn, "forge-47.2.0", b"partial-v1", t1).unwrap(),
        1
    );
    let row = vm::get_partial_version_info(&conn, "forge-47.2.0")
        .unwrap()
        .unwrap();
    ...
    let t2 = DbDateTime(from_millis(1_784_557_692_104).unwrap());
    vm::upsert_partial_version_info(&conn, "forge-47.2.0", b"partial-v2", t2).unwrap();
    let row = vm::get_partial_version_info(&conn, "forge-47.2.0")
        .unwrap()
        .unwrap();
...
fn lwjgl_meta_upsert_inserts_then_updates_payload_and_freshness() {
    ...
    let t1 = DbDateTime(from_millis(1_700_000_000_000).unwrap());
    assert_eq!(
        vm::upsert_lwjgl_meta(&conn, "1.20-lwjgl3", b"lwjgl-v1", t1).unwrap(),
        1
    );
    let row = vm::get_lwjgl_meta(&conn, "1.20-lwjgl3").unwrap().unwrap();
    assert_eq!(row.lwjgl, b"lwjgl-v1");
    assert_eq!(row.last_updated_at, t1.0);

    let t2 = DbDateTime(from_millis(1_784_557_692_104).unwrap());
    vm::upsert_lwjgl_meta(&conn, "1.20-lwjgl3", b"lwjgl-v2", t2).unwrap();
    let row = vm::get_lwjgl_meta(&conn, "1.20-lwjgl3").unwrap().unwrap();
...
fn assets_meta_upsert_inserts_then_updates_payload_and_freshness() {
    ...
    let t1 = DbDateTime(from_millis(1_700_000_000_000).unwrap());
    assert_eq!(
        vm::upsert_assets_meta(&conn, "17", b"assets-v1", t1).unwrap(),
        1
    );
    let row = vm::get_assets_meta(&conn, "17").unwrap().unwrap();
    assert_eq!(row.assets_index, b"assets-v1");
    assert_eq!(row.last_updated_at, t1.0);

    let t2 = DbDateTime(from_millis(1_784_557_692_104).unwrap());
    vm::upsert_assets_meta(&conn, "17", b"assets-v2", t2).unwrap();
    let row = vm::get_assets_meta(&conn, "17").unwrap().unwrap();
...
fn assets_meta_get_missing_returns_none() {
    let (_d, conn) = migrated_db();
    assert!(vm::get_assets_meta(&conn, "nope").unwrap().is_none());
}
