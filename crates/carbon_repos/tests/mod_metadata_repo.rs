...
fn insert_and_find_by_64_byte_hashes() {
    ...
    let row = mm::find_metadata_by_hashes(&conn, &s512, 42)
        .unwrap()
    ...
    assert!(mm::find_metadata_by_hashes(&conn, &s512, 99).unwrap().is_none());
    // Different sha512 -> no match.
    assert!(mm::find_metadata_by_hashes(&conn, &sha512(0x01), 42).unwrap().is_none());
}

#[test]
fn insert_metadata_accepts_all_none_optionals() {
    let (_d, conn) = migrated_db();
    mm::insert_metadata(&conn, "m", 1, &sha512(1), &[1, 2], "", None, None, None, None, None, ts(1))
        .unwrap();
    let row = mm::find_metadata_by_hashes(&conn, &sha512(1), 1).unwrap().unwrap();
...
fn insert_local_image_stores_blob() {
    let (_d, conn) = migrated_db();
    mm::insert_metadata(&conn, "m", 1, &sha512(1), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();
    assert_eq!(mm::insert_local_image(&conn, "m", &[9, 8, 7]).unwrap(), 1);
...
fn gc_removes_only_true_orphans() {
    ...
    mm::insert_metadata(&conn, "orphan", 1, &sha512(1), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();
    // referenced by an instance file cache
    mm::insert_metadata(&conn, "inst", 2, &sha512(2), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();
    insert_mfc(&conn, 1, "a.jar", "inst");
    // referenced by a server file cache
    mm::insert_metadata(&conn, "srv", 3, &sha512(3), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();
    conn.execute(
    ...
    assert_eq!(mm::gc_orphan_metadata(&conn).unwrap(), 1, "only the orphan is deleted");
    assert!(mm::find_metadata_by_hashes(&conn, &sha512(1), 1).unwrap().is_none());
    assert!(mm::find_metadata_by_hashes(&conn, &sha512(2), 2).unwrap().is_some());
    assert!(mm::find_metadata_by_hashes(&conn, &sha512(3), 3).unwrap().is_some());
}
...
fn cf_upsert(conn: &Connection, metadata_id: &str, project_id: i32, file_id: i32, name: &str, cached_at: i64) -> String {
    mm::upsert_cf_mod_cache(
        conn, 0, project_id, file_id, name, "v", "slug", "sum", "auth", 2, "paths", ts(cached_at), metadata_id,
    )
    .unwrap()
}
...
fn cf_upsert_inserts_then_composite_conflict_keeps_original_metadata_id() {
    let (_d, conn) = migrated_db();
    mm::insert_metadata(&conn, "meta-A", 1, &sha512(1), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();
    mm::insert_metadata(&conn, "meta-B", 2, &sha512(2), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();
    ...
    let row = mm::get_cf_cache_by_metadata(&conn, "meta-A").unwrap().unwrap();
    assert_eq!(row.name, "second");
    assert_eq!(row.cached_at, ts(200).0);
    assert!(mm::get_cf_cache_by_metadata(&conn, "meta-B").unwrap().is_none());
    let count: i64 = conn
...
fn cf_get_by_metadata_reads_cached_at() {
    ...
    let row = mm::get_cf_cache_by_metadata(&conn, "m").unwrap().unwrap();
    assert_eq!(row.project_id, 1);
    assert_eq!(row.file_id, 2);
    assert_eq!(row.cached_at, ts(12345).0);
    assert!(mm::get_cf_cache_by_metadata(&conn, "nope").unwrap().is_none());
}
...
fn mr_upsert_composite_conflict_keeps_original_metadata_id() {
    let (_d, conn) = migrated_db();
    mm::insert_metadata(&conn, "mA", 1, &sha512(1), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();
    mm::insert_metadata(&conn, "mB", 2, &sha512(2), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();

    let r = mm::upsert_mr_mod_cache(
        &conn, "sha", "proj", "ver", "t1", "v", "slug", "d", "a", 2, "", "f.jar", "http://u", ts(1), "mA",
    )
    .unwrap();
    assert_eq!(r, "mA");

    let r = mm::upsert_mr_mod_cache(
        &conn, "sha", "proj", "ver", "t2", "v", "slug", "d", "a", 2, "", "f.jar", "http://u", ts(9), "mB",
    )
    .unwrap();
    assert_eq!(r, "mA", "composite (projectId, versionId) conflict preserves metadataId");

    let row = mm::get_mr_cache_by_metadata(&conn, "mA").unwrap().unwrap();
...
fn cf_image_upsert_marks_stale_and_download_clears_it() {
    ...
    assert_eq!(mm::upsert_cf_image(&conn, "m", "url-1").unwrap(), 1);
    ...
    assert_eq!(mm::mark_cf_image_downloaded(&conn, "m", &[1, 2, 3]).unwrap(), 1);
    ...
    assert_eq!(mm::upsert_cf_image(&conn, "m", "url-2").unwrap(), 1);
    let (url, data, up): (String, Option<Vec<u8>>, i32) = conn
        .query_row(
            "SELECT url, data, upToDate FROM CurseForgeModImageCache WHERE metadataId = 'm'",
            [],
...
fn mr_image_upsert_and_download() {
    ...
    mm::insert_metadata(&conn, "m", 1, &sha512(1), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();
    mm::upsert_mr_image(&conn, "m", "u").unwrap();
    mm::mark_mr_image_downloaded(&conn, "m", &[7]).unwrap();
    let (data, up): (Option<Vec<u8>>, i32) = conn
        .query_row(
...
fn cf_export_enrich_includes_modrinth_cross_reference() {
    ...
    mm::insert_metadata(&conn, "both", 1, &sha512(1), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();
    cf_upsert(&conn, "both", 111, 222, "cf-name", 1);
    mm::upsert_mr_mod_cache(
        &conn, "sha", "mrproj", "mrver", "t", "v", "mrslug", "d", "a", 2, "", "f", "u", ts(1), "both",
    )
    .unwrap();
    // metadata with only a CF cache row -> mr_* columns NULL.
    mm::insert_metadata(&conn, "cfonly", 2, &sha512(2), &[1], "", None, None, None, None, None, ts(1))
        .unwrap();
    cf_upsert(&conn, "cfonly", 333, 444, "cf2", 1);

    let both = mm::get_cf_export_enrich_by_project(&conn, 111).unwrap();
    ...
    let cfonly = mm::get_cf_export_enrich_by_project(&conn, 333).unwrap();
    assert_eq!(cfonly.len(), 1);
    assert_eq!(cfonly[0].mr_project_id, None);

    let mr = mm::get_mr_export_enrich_by_project(&conn, "mrproj").unwrap();
    assert_eq!(mr.len(), 1);
