...
fn java_crud_roundtrip() {
    ...
    assert_eq!(
        j::get_all_java_conn(&wg(&mut conn)).unwrap(),
        vec![sample()]
    );
    assert_eq!(
        j::get_java_by_id_conn(&wg(&mut conn), "id-1").unwrap(),
        Some(sample())
    );
    assert_eq!(
        j::get_java_by_path_conn(&wg(&mut conn), "/usr/bin/java").unwrap(),
        Some(sample())
    );
    assert_eq!(j::count_java_conn(&wg(&mut conn)).unwrap(), 1);
    assert_eq!(
        j::set_java_validity_conn(&wg(&mut conn), "id-1", false).unwrap(),
        1
    );
    assert!(
        !j::get_java_by_id_conn(&wg(&mut conn), "id-1")
            .unwrap()
            .unwrap()
            .is_valid
    );
...
fn java_profile_crud_roundtrip() {
    ...
    assert_eq!(
        j::upsert_profile_conn(&wg(&mut conn), "gaming", false).unwrap(),
        1
    );
    assert_eq!(
        j::set_profile_java_conn(&wg(&mut conn), "gaming", Some("id-1")).unwrap(),
        1
    );
    let p = j::get_profile_conn(&wg(&mut conn), "gaming")
        .unwrap()
        .unwrap();
    assert_eq!(p.java_id.as_deref(), Some("id-1"));
    assert_eq!(j::get_all_profiles_conn(&wg(&mut conn)).unwrap().len(), 1);
    assert_eq!(
        j::set_profile_java_conn(&wg(&mut conn), "gaming", None).unwrap(),
        1
    );
    assert_eq!(
        j::get_profile_conn(&wg(&mut conn), "gaming")
            .unwrap()
            .unwrap()
            .java_id,
        None
...
fn sample2() -> j::JavaRow {
    j::JavaRow {
        id: "id-2".into(),
        path: "/opt/java/bin/java".into(),
        major: 8,
        full_version: "8.0.392".into(),
        r#type: "Managed".into(),
        os: "linux".into(),
        arch: "x64".into(),
        vendor: "Temurin".into(),
...
fn java_lookups_and_updates_by_type_and_path() {
    ...
    assert!(
        j::get_java_by_type_conn(&wg(&mut conn), "Custom")
            .unwrap()
            .is_empty()
    );
    ...
    assert!(
        !j::get_java_by_id_conn(&wg(&mut conn), "id-2")
            .unwrap()
            .unwrap()
            .is_valid
    );
    ...
    assert_eq!(
        j::set_java_validity_by_path_conn(&wg(&mut conn), "/nope", true).unwrap(),
        0
    );

    // update_java_component rewrites the component fields and forces isValid = 1.
    assert_eq!(
        j::update_java_component_conn(
            &wg(&mut conn),
            "id-2",
            21,
            "21.0.2",
    ...
    let row = j::get_java_by_id_conn(&wg(&mut conn), "id-2")
        .unwrap()
        .unwrap();
    ...
    assert!(
        row.is_valid,
        "update_java_component marks the component valid"
    );

    // delete_java_by_path removes exactly the row at that path.
    assert_eq!(
        j::delete_java_by_path_conn(&wg(&mut conn), "/usr/bin/java").unwrap(),
        1
    );
    assert_eq!(j::count_java_conn(&wg(&mut conn)).unwrap(), 1);
    assert_eq!(
...
fn read_class_fns_accept_read_guards() {
    ...
    assert_eq!(
        j::get_java_by_id_conn(&rg(&conn), "id-1").unwrap(),
        Some(sample())
    );
    assert_eq!(
        j::get_java_by_path_conn(&rg(&conn), "/usr/bin/java").unwrap(),
        Some(sample())
    );
