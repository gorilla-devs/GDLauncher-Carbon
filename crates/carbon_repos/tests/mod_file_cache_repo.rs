...
fn upsert_inserts_then_conflict_preserves_id_and_refreshes_freshness() {
    ...
    let n = mfc::upsert_mod_file_cache(&conn, 7, "a.jar", 100, true, "mods", "meta-1", ts(1000))
        .unwrap();
    assert_eq!(n, 1);

    let row = mfc::get_mod_file_cache_by_instance_filename(&conn, 7, "a.jar")
        .unwrap()
        .unwrap();
    ...
    mfc::upsert_mod_file_cache(&conn, 7, "a.jar", 200, false, "shaders", "meta-2", ts(5000))
        .unwrap();
    ...
    assert_eq!(mfc::get_mod_files_by_instance(&conn, 7).unwrap().len(), 1);
}
...
fn upsert_generates_distinct_uuids_per_row() {
    ...
    mfc::upsert_mod_file_cache(&conn, 1, "a.jar", 1, true, "mods", "m", ts(1)).unwrap();
    mfc::upsert_mod_file_cache(&conn, 1, "b.jar", 1, true, "mods", "m", ts(1)).unwrap();
    let rows = mfc::get_mod_files_by_instance(&conn, 1).unwrap();
...
fn server_upsert_conflict_preserves_id_and_freshness() {
    let (_d, conn) = migrated_db();
    insert_metadata(&conn, "m", 1, b"a", b"b", None, None);
    mfc::upsert_server_mod_file_cache(&conn, 3, "a.jar", 10, true, "mods", "m", ts(1000)).unwrap();
    let row = mfc::get_server_mod_files_by_server(&conn, 3).unwrap().remove(0);
    let id = row.id.clone();
    assert_eq!(row.last_updated_at, ts(1000).0);

    mfc::upsert_server_mod_file_cache(&conn, 3, "a.jar", 20, false, "datapacks", "m", ts(2000))
        .unwrap();
    let row = mfc::get_server_mod_file_cache_by_id(&conn, &id).unwrap().unwrap();
    assert_eq!(row.id, id);
...
fn update_enabled_sets_flag_and_freshness() {
    ...
    insert_metadata(&conn, "m", 1, b"a", b"b", None, None);
    mfc::upsert_mod_file_cache(&conn, 1, "a.jar", 1, true, "mods", "m", ts(1)).unwrap();
    let id = mfc::get_mod_files_by_instance(&conn, 1).unwrap().remove(0).id;

    assert_eq!(mfc::update_mod_file_enabled(&conn, &id, false, ts(9999)).unwrap(), 1);
    let row = mfc::get_mod_file_cache_by_id(&conn, &id).unwrap().unwrap();
    assert_eq!(row.enabled, false);
    assert_eq!(row.last_updated_at, ts(9999).0);

    assert_eq!(mfc::update_server_mod_file_enabled(&conn, "nope", true, ts(1)).unwrap(), 0);
}
...
fn deletes_scope_correctly() {
    ...
    mfc::upsert_mod_file_cache(&conn, 1, "a.jar", 1, true, "mods", "m", ts(1)).unwrap();
    mfc::upsert_mod_file_cache(&conn, 1, "b.jar", 1, true, "mods", "m", ts(1)).unwrap();
    mfc::upsert_mod_file_cache(&conn, 2, "c.jar", 1, true, "mods", "m", ts(1)).unwrap();

    assert_eq!(mfc::delete_mod_file_cache_by_instance_filename(&conn, 1, "a.jar").unwrap(), 1);
    assert_eq!(mfc::get_mod_files_by_instance(&conn, 1).unwrap().len(), 1);
    assert_eq!(mfc::delete_mod_file_cache_by_instance(&conn, 1).unwrap(), 1);
    assert_eq!(mfc::get_mod_files_by_instance(&conn, 1).unwrap().len(), 0);
    // instance 2 untouched
    assert_eq!(mfc::get_mod_files_by_instance(&conn, 2).unwrap().len(), 1);
}
...
fn full_row_decodes_populated_and_bare_metadata() {
    ...
    .unwrap();
    mfc::upsert_mod_file_cache(&conn, 1, "full.jar", 42, true, "mods", "full", ts(1)).unwrap();

    // Bare: metadata only, no platform rows, no local image.
    insert_metadata(&conn, "bare", 222, b"b512", b"b1", None, None);
    mfc::upsert_mod_file_cache(&conn, 1, "bare.jar", 7, false, "resourcepacks", "bare", ts(1))
        .unwrap();

    let mut rows = mfc::get_instance_mods_full(&conn, 1).unwrap();
...
fn cf_image_presence_checks_data_for_instance_but_relation_for_server() {
    ...
    .unwrap();
    mfc::upsert_mod_file_cache(&conn, 1, "a.jar", 1, true, "mods", "m", ts(1)).unwrap();
    mfc::upsert_server_mod_file_cache(&conn, 1, "a.jar", 1, true, "mods", "m", ts(1)).unwrap();

    let inst = mfc::get_instance_mods_full(&conn, 1).unwrap().remove(0);
    assert_eq!(inst.has_cf_image, false, "instance checks data presence");

    let srv = mfc::get_server_mods_full(&conn, 1).unwrap().remove(0);
...
fn icon_data_returns_all_three_blobs() {
    ...
    let icon = mfc::get_instance_mod_icon_data(&conn, &id).unwrap().unwrap();
...
fn cf_refresh_selects_missing_and_stale_excluding_worlds() {
    ...
    mfc::upsert_mod_file_cache(&conn, 1, "missing.jar", 1, true, "mods", "missing", ts(1)).unwrap();
    ...
    mfc::upsert_mod_file_cache(&conn, 1, "fresh.jar", 1, true, "mods", "fresh", ts(1)).unwrap();
    ...
    mfc::upsert_mod_file_cache(&conn, 1, "stale.jar", 1, true, "mods", "stale", ts(1)).unwrap();
    // world addon with no cf -> excluded by addonType filter
    insert_metadata(&conn, "world", 4, b"a", b"b", None, None);
    mfc::upsert_mod_file_cache(&conn, 1, "world1", 1, true, "worlds", "world", ts(1)).unwrap();

    let mut ids: Vec<String> = mfc::instance_mods_needing_cf_refresh(&conn, 1, ts(5000))
        .unwrap()
        .into_iter()
        .map(|r| r.metadata_id)
        .collect();
...
fn mr_refresh_and_server_variants_work() {
    ...
    mfc::upsert_server_mod_file_cache(&conn, 9, "a.jar", 1, true, "mods", "m1", ts(1)).unwrap();
    // fresh mr -> excluded
    insert_metadata(&conn, "m2", 2, b"a", b"b", None, None);
    insert_mr(&conn, "m2", "p", "v", "t", "s", "f", 10_000);
    mfc::upsert_server_mod_file_cache(&conn, 9, "b.jar", 1, true, "mods", "m2", ts(1)).unwrap();

    let ids: Vec<String> = mfc::server_mods_needing_mr_refresh(&conn, 9, ts(5000))
        .unwrap()
        .into_iter()
        .map(|r| r.metadata_id)
        .collect();
...
fn stale_cf_logo_selects_only_uptodate_zero() {
    ...
    let rows = mfc::instance_mods_stale_cf_logo(&conn, 1).unwrap();
    ...
    conn.execute("UPDATE CurseForgeModImageCache SET upToDate = 1 WHERE metadataId = 'm'", [])
        .unwrap();
    assert_eq!(mfc::instance_mods_stale_cf_logo(&conn, 1).unwrap().len(), 0);
}
...
fn exists_by_project_matches_only_installed() {
    ...
    mfc::upsert_mod_file_cache(&conn, 5, "a.jar", 1, true, "mods", "m", ts(1)).unwrap();

    assert!(mfc::instance_mod_exists_by_cf_project(&conn, 5, 4242).unwrap().is_some());
    assert!(mfc::instance_mod_exists_by_cf_project(&conn, 5, 9999).unwrap().is_none());
    // wrong instance
    assert!(mfc::instance_mod_exists_by_cf_project(&conn, 6, 4242).unwrap().is_none());
}
...
fn update_ids_and_platform_ids() {
    ...
    let cf = mfc::get_instance_mod_cf_ids(&conn, &id).unwrap().unwrap();
    assert_eq!((cf.cf_project_id, cf.cf_file_id), (Some(10), Some(20)));
    let mr = mfc::get_instance_mod_mr_ids(&conn, &id).unwrap().unwrap();
    assert_eq!(mr.mr_project_id.as_deref(), Some("mp"));
    let both = mfc::get_instance_mod_update_ids(&conn, &id).unwrap().unwrap();
...
fn enabled_modids_only() {
    ...
    mfc::upsert_mod_file_cache(&conn, 1, "on.jar", 1, true, "mods", "on", ts(1)).unwrap();
    mfc::upsert_mod_file_cache(&conn, 1, "off.jar", 1, false, "mods", "off", ts(1)).unwrap();

    let modids: Vec<String> = mfc::get_enabled_instance_mod_modids(&conn, 1)
        .unwrap()
        .into_iter()
...
fn export_queries_shape() {
    ...
    mfc::upsert_mod_file_cache(&conn, 1, "a.jar", 321, true, "mods", "m", ts(1)).unwrap();
    ...
    mfc::upsert_mod_file_cache(&conn, 1, "d.zip", 1, true, "datapacks", "dp", ts(1)).unwrap();

    let shared = mfc::get_instance_export_mods(&conn, 1).unwrap();
    assert_eq!(shared.len(), 1);
    assert_eq!(shared[0].cf_name.as_deref(), Some("cfname"));
    assert_eq!(shared[0].mr_title.as_deref(), Some("mrtitle"));

    let mr = mfc::get_instance_mr_export_files(&conn, 1).unwrap();
    ...
    let gdl = mfc::get_instance_gdl_export_files(&conn, 1).unwrap();
    ...
    let cf = mfc::get_instance_cf_export_files(&conn, 1).unwrap();
