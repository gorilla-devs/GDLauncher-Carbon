...
fn cf_get_missing_returns_none() {
    ...
    assert!(mp::get_cf_modpack(&conn, 1, 2).unwrap().is_none());
}
...
fn cf_upsert_inserts_then_get_reads_it_back_with_no_logo() {
    let (_d, conn) = migrated_db();
    assert_eq!(
        mp::upsert_cf_modpack(&conn, 100, 200, "RLCraft", "2.9.3", "rlcraft", ts(1000)).unwrap(),
        1
    );

    let row = mp::get_cf_modpack(&conn, 100, 200).unwrap().expect("row inserted");
...
fn cf_upsert_refreshes_updated_at_on_conflict() {
    let (_d, conn) = migrated_db();
    mp::upsert_cf_modpack(&conn, 1, 1, "A", "v1", "a", ts(1_000)).unwrap();
    let first = mp::get_cf_modpack(&conn, 1, 1).unwrap().unwrap();
    ...
    mp::upsert_cf_modpack(&conn, 1, 1, "A2", "v2", "a2", ts(2_000)).unwrap();
    let second = mp::get_cf_modpack(&conn, 1, 1).unwrap().unwrap();
...
fn cf_image_upsert_then_get_logo_and_joined_read() {
    let (_d, conn) = migrated_db();
    mp::upsert_cf_modpack(&conn, 5, 9, "Pack", "v", "slug", ts(1)).unwrap();

    // No image row yet.
    assert!(mp::get_cf_modpack_logo(&conn, 5, 9).unwrap().is_none());
    let joined = mp::get_cf_modpack(&conn, 5, 9).unwrap().unwrap();
    assert!(!joined.has_logo);
    assert_eq!(joined.logo_data, None);

    // Insert image with data.
    assert_eq!(
        mp::upsert_cf_modpack_image(&conn, 5, 9, "http://icon", Some(&[1, 2, 3])).unwrap(),
        1
    );
    let img = mp::get_cf_modpack_logo(&conn, 5, 9).unwrap().unwrap();
    ...
    assert_eq!(
        mp::upsert_cf_modpack_image(&conn, 5, 9, "", None).unwrap(),
        1
    );
    let joined = mp::get_cf_modpack(&conn, 5, 9).unwrap().unwrap();
...
fn mr_get_missing_returns_none() {
    let (_d, conn) = migrated_db();
    assert!(mp::get_mr_modpack(&conn, "p", "v").unwrap().is_none());
}
...
fn mr_upsert_inserts_then_get_reads_it_back() {
    let (_d, conn) = migrated_db();
    assert_eq!(
        mp::upsert_mr_modpack(&conn, "1KVo5zza", "HH3vor7X", "Fabulously Optimized", "6.0", "fabulously-optimized", ts(500))
            .unwrap(),
        1
    );
    let row = mp::get_mr_modpack(&conn, "1KVo5zza", "HH3vor7X").unwrap().expect("row inserted");
...
fn mr_upsert_refreshes_updated_at_on_conflict() {
    let (_d, conn) = migrated_db();
    mp::upsert_mr_modpack(&conn, "p", "v", "A", "v1", "a", ts(1_000)).unwrap();
    mp::upsert_mr_modpack(&conn, "p", "v", "A2", "v2", "a2", ts(2_000)).unwrap();
    let row = mp::get_mr_modpack(&conn, "p", "v").unwrap().unwrap();
...
fn mr_image_upsert_then_get_logo_and_joined_read() {
    let (_d, conn) = migrated_db();
    mp::upsert_mr_modpack(&conn, "p", "v", "Pack", "v", "slug", ts(1)).unwrap();

    assert!(mp::get_mr_modpack_logo(&conn, "p", "v").unwrap().is_none());

    assert_eq!(
        mp::upsert_mr_modpack_image(&conn, "p", "v", "http://icon", Some(&[9, 8])).unwrap(),
        1
    );
    let img = mp::get_mr_modpack_logo(&conn, "p", "v").unwrap().unwrap();
    assert_eq!(img.data, Some(vec![9, 8]));

    let joined = mp::get_mr_modpack(&conn, "p", "v").unwrap().unwrap();
    assert!(joined.has_logo);
...
fn distinct_composite_keys_do_not_collide() {
    let (_d, conn) = migrated_db();
    mp::upsert_cf_modpack(&conn, 1, 1, "one", "v", "s", ts(1)).unwrap();
    mp::upsert_cf_modpack(&conn, 1, 2, "two", "v", "s", ts(1)).unwrap();
    mp::upsert_cf_modpack(&conn, 2, 1, "three", "v", "s", ts(1)).unwrap();

    assert_eq!(mp::get_cf_modpack(&conn, 1, 1).unwrap().unwrap().modpack_name, "one");
    assert_eq!(mp::get_cf_modpack(&conn, 1, 2).unwrap().unwrap().modpack_name, "two");
    assert_eq!(mp::get_cf_modpack(&conn, 2, 1).unwrap().unwrap().modpack_name, "three");
