...
fn cf_get_missing_returns_none() {
    ...
    assert!(
        mp::get_cf_modpack_conn(&wg(&mut conn), 1, 2)
            .unwrap()
            .is_none()
    );
}
...
fn cf_upsert_inserts_then_get_reads_it_back_with_no_logo() {
    ...
    let row = mp::get_cf_modpack_conn(&wg(&mut conn), 100, 200)
        .unwrap()
        .expect("row inserted");
...
fn cf_upsert_refreshes_updated_at_on_conflict() {
    ...
    let first = mp::get_cf_modpack_conn(&wg(&mut conn), 1, 1)
        .unwrap()
        .unwrap();
    ...
    let second = mp::get_cf_modpack_conn(&wg(&mut conn), 1, 1)
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        second.updated_at,
        ts(2_000).0,
        "updatedAt must be refreshed by the upsert"
    );
    assert_ne!(second.updated_at, first.updated_at);

    let count: i64 = conn
        .query_row("SELECT COUNT(*) FROM CurseForgeModpackCache", [], |r| {
            r.get(0)
        })
        .unwrap();
...
fn cf_image_upsert_then_get_logo_and_joined_read() {
    ...
    // No image row yet.
    assert!(
        mp::get_cf_modpack_logo_conn(&wg(&mut conn), 5, 9)
            .unwrap()
            .is_none()
    );
    let joined = mp::get_cf_modpack_conn(&wg(&mut conn), 5, 9)
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        mp::upsert_cf_modpack_image_conn(&wg(&mut conn), 5, 9, "http://icon", Some(&[1, 2, 3]))
            .unwrap(),
        1
    );
    let img = mp::get_cf_modpack_logo_conn(&wg(&mut conn), 5, 9)
        .unwrap()
        .unwrap();
    ...
    assert!(
        joined.has_logo,
        "row still exists after re-upsert with no bytes"
    );
    assert_eq!(joined.logo_data, None);
...
fn mr_get_missing_returns_none() {
    let (_d, mut conn) = migrated_db();
    assert!(
        mp::get_mr_modpack_conn(&wg(&mut conn), "p", "v")
            .unwrap()
            .is_none()
    );
}
...
fn mr_upsert_inserts_then_get_reads_it_back() {
    ...
    let row = mp::get_mr_modpack_conn(&wg(&mut conn), "1KVo5zza", "HH3vor7X")
        .unwrap()
        .expect("row inserted");
...
fn mr_upsert_refreshes_updated_at_on_conflict() {
    ...
    let row = mp::get_mr_modpack_conn(&wg(&mut conn), "p", "v")
        .unwrap()
        .unwrap();
    ...
    let count: i64 = conn
        .query_row("SELECT COUNT(*) FROM ModrinthModpackCache", [], |r| {
            r.get(0)
        })
        .unwrap();
    assert_eq!(count, 1);
...
fn mr_image_upsert_then_get_logo_and_joined_read() {
    ...
    assert!(
        mp::get_mr_modpack_logo_conn(&wg(&mut conn), "p", "v")
            .unwrap()
            .is_none()
    );

    assert_eq!(
        mp::upsert_mr_modpack_image_conn(&wg(&mut conn), "p", "v", "http://icon", Some(&[9, 8]))
            .unwrap(),
        1
    );
    let img = mp::get_mr_modpack_logo_conn(&wg(&mut conn), "p", "v")
        .unwrap()
        .unwrap();
    assert_eq!(img.data, Some(vec![9, 8]));

    let joined = mp::get_mr_modpack_conn(&wg(&mut conn), "p", "v")
        .unwrap()
        .unwrap();
    assert!(joined.has_logo);
    ...
    let count: i64 = conn
        .query_row("SELECT COUNT(*) FROM ModrinthModpackImageCache", [], |r| {
            r.get(0)
        })
        .unwrap();
    assert_eq!(count, 1);
...
fn distinct_composite_keys_do_not_collide() {
    ...
    assert_eq!(
        mp::get_cf_modpack_conn(&wg(&mut conn), 1, 1)
            .unwrap()
            .unwrap()
            .modpack_name,
        "one"
    ...
    assert_eq!(
        mp::get_cf_modpack_conn(&wg(&mut conn), 2, 1)
            .unwrap()
            .unwrap()
            .modpack_name,
        "three"
