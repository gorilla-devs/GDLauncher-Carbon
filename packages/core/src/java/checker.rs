...
pub async fn find_java_paths() -> Vec<PathBuf> {
    ...
    // Remove duplicates
    javas = javas.into_iter().filter(|java_path| java_path.to_str().is_some()).collect();
    javas.sort_by(|a, b| a.to_str().unwrap().to_lowercase().cmp(&b.to_str().unwrap().to_lowercase()));
    javas.dedup();
