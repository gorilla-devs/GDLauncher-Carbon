...
## Modrinth support
We talked with their team but sadly we weren't able to find to a common agreement with them for the time being.
This is because we need to proxy and aggregate all requests through our servers to reduce the load on the client and provide a better experience for the end user, but they said they won't support this use case, as all requests should be done client side.

We are still looking into other options, but for now all of them will result in a worse experience for the end user, so we will probably not support Modrinth in the first release of GDLauncher Carbon.
