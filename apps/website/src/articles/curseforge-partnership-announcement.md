...
GorillaDevs, the entity behind GDLauncher, is now officially a company. As such, we will be able to get access to a lot of things that were just out of touch before. We can also now hire people to help us develop GDLauncher, which we have wanted to do for a while now.
...
This partnership will not impact the current launcher too much.
...
After GDLauncher Carbon is ready to go to production, the current release of GDLauncher will be archived. We will probably change the license to MIT, so anyone will be free to do (mostly) whatever they want with it!
...
We might add monthly subscriptions to access non-essential features and hide ads.
...
Well... It wasn't easy to choose between the two. Rust is a great language, but it's still not mature enough, in our opinion. Golang is a bit more mature and has a much larger community, so it's easier to find help, documentation, and libraries. We also believe that Golang is a better fit for our use case as it better represents our coding style.
...
Moreover, it fits our ideology of "Easiness of programming" much better, instead of trying to overengineer every piece of code by using a low-level language full of clever tricks that are often unnecessary.
...
First of all, if you didn't do it already (SHAME ON YOU!!), you could come over to our [Discord](https://discord.gdlauncher.com) and make suggestions on what you would like to see implemented!
...
- Apply to our hiring selection at [here](https://gdlauncher.com/careers) to work with us full time! We are hiring designers, and Golang and JavaScript engineers
