...
import { mapMcTypeToColor, mcVersions } from "@/utils/mcVersion";
...
const InstanceCreation = (props: ModalProps) => {
  ...
  const lastMcVersioId = () => mappedMcVersions()?.[0]?.id as string;

  const selectedMcVersion = () => mcVersion() || lastMcVersioId();

  createEffect(() => {
    if (forgeVersionsQuery.data && isForge()) {
      const versions = forgeVersionsQuery?.data?.gameVersions.find(
        (v) => v.id === selectedMcVersion()
      )?.loaders;
  ...
  const autoGeneratedName = () =>
    `${loader() || "Vanilla"} ${selectedMcVersion()}`;
      ...
      <div class="flex flex-col justify-between scrollbar-hide w-120 overflow-y-scroll h-136">
        <div class="flex flex-col justify-between gap-4 h-full p-5">
          <span class="flex flex-col justify-between gap-4">
            ...
            <ModloadersPicker />
            <Switch>
              <Match when={mappedMcVersions().length > 0}>
                <MainContent />
              </Match>
              <Match when={mappedMcVersions().length === 0}>
                <p>
                  <Trans key="error.no_mc_versions" />
                </p>
              </Match>
            </Switch>
          </span>
