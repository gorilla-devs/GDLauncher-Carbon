...
const Custom = (props: Pick<ModalProps, "data">) => {
  ...
  const mapLoaderToHashMap = (loader: CFFEModLoaderType | undefined) => {
    switch (loader) {
      case "forge":
        return forgeHasmap;
      case "fabric":
        return fabricHashmap;
      case "quilt":
        return quiltHashmap;
      default:
        return forgeHasmap;
  ...
  createEffect(() => {
    ...
    const hashMap = mapLoaderToHashMap(loader());

    const mappedVersions = filteredData.map((item) => {
      return { ...item, hasModloader: hashMap.has(item.id) };
    });
  ...
  return (
    <div class="flex flex-col justify-between gap-4 h-full p-5 box-border">
