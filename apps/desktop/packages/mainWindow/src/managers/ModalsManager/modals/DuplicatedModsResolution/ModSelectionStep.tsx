    ...
    <div class="flex h-full flex-col">
      <div class="flex shrink-0 flex-col gap-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-lightSlate-600 m-0 text-sm">
                <Trans
                  key="instance:_trn_duplicates.selection.step_indicator"
                  options={{
        ...
        <p class="text-lightSlate-700 text-sm">
          <Trans key="instance:_trn_duplicates.selection.instruction" />
        </p>
      </div>
                ...
                >
                  <div class="flex w-full flex-col gap-1">
                    ...
                    <div class="text-lightSlate-600 mt-1 flex gap-4 text-xs">
                      <span>
                        <Trans key="instance:_trn_duplicates.selection.version_label" />{" "}
                        {version.version}
                      </span>
                    </div>
                  </div>
      ...
      <div class="mt-6 flex shrink-0 justify-between">
        <Button type="secondary" size="large" onClick={() => props.prevStep()}>
          <Trans key="instance:_trn_duplicates.selection.button_back" />
        </Button>
        <Button type="primary" size="large" onClick={() => props.nextStep()}>
          {props.currentModIndex + 1 === props.totalMods ? (
            <Trans key="instance:_trn_duplicates.selection.button_continue" />
          ) : (
            <Trans key="instance:_trn_duplicates.selection.button_next" />
          )}
        </Button>
      </div>
    </div>
