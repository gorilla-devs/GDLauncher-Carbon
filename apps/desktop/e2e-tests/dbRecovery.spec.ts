 ...
 * future-versioned `gdl_conf.db` with `helpers/dbSeed.ts` (Task 3's helper,
 * cross-checked against the real Rust classifier — see that file's own
 ...
 * a `process()` to attach a listener to — confirmed empirically (see
 * task-4-report.md): a first attempt using this capture for `DB_CORRUPT`
 ...
 * `DB_DOWNGRADED` (see below) — the one status this file needs that was
 * previously not threaded anywhere observable once startup moved on.
 ...
 * read back from the real running app's DOM (see task-4-report.md), not
 ...
 * state it could ever apply to, not skipped — see task-4-report.md for the
 * full trace.
 *
  ...
  if (process.platform === "win32") {
    console.warn(
      "cleanupRelaunchSiblings: no-op on win32 — a relaunched sibling process " +
        "is left running for this job's own VM lifetime; see task-4-report.md"
    )
    return
  }
