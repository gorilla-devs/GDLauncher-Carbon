 ...
 * `get_migrations()` in `crates/carbon_repos/src/lib.rs` — the single
 * source of truth binding directory to MigrationDef, per its own doc
 * comment — rather than trusting `dbSeed.ts`'s own
 ...
 * migrations authored after the floor).
 */
...
describe("seedDatabase", () => {
  it("clears a stale WAL sidecar from a previous run before seeding", async () => {
    ...
    // planted state — exactly the failure mode this seeding path avoids.
  ...
  }, 30_000) // Seeds a real database per state; the default is too tight.
})
