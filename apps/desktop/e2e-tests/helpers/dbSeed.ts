 ...
 *   matches this binary's own schema byte-for-byte. The brief's "additive"
 *   wording does not match the code it says defines the classification
 *   exactly, so the code wins.
 ...
 * carbon_repos/src/lib.rs`'s `get_migrations()` is documented (root
 * `CLAUDE.md`) as "the single source of truth binding directory ->
 * MigrationDef", one `historical_migration!(...)` entry per directory
