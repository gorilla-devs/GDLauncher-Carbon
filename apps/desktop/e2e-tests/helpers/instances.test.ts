...
// Minecraft release (a live DOM probe against 26.2 returned exactly this
// many options).
const OPTIONS = [
  "26.2-65.0.9",
  "26.2-65.0.8",
...
describe("loaderVersionIdFromTestId", () => {
  ...
  it("recovers the id of whichever option Custom.tsx pre-selects (index 0), unaffected by its checkmark markup", () => {
    ...
    // Recovering ids via `allTextContents()` returns that one option's "id"
    // as `"Checked1.20.1-47.4.22"` — never equal to its own `data-testid`
    // suffix, which would break both a
