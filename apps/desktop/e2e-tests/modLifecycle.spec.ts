 ...
 * verified on disk (Task 2's `modVerify.ts`), never by the UI's own
 ...
 * only verified live against Modrinth (see task-5-report.md) — CurseForge's
...
test.describe("mod lifecycle", () => {
  ...
  test("disables an installed mod", async ({ installedInstance }) => {
    ...
    try {
      ...
      // The real disk representation (Task 2), not the switch's own state —
  ...
  /**
   * The awkward one (see task-5-brief.md). A real update needs a file
   ...
   * timing races this took several live runs to pin down (task-5-report.md):
  ...
  test("updates an installed mod to a newer version", async ({
    ...
    try {
      ...
      // it already guarantees, proving nothing (see task-5-report.md's
      // "Fix round 1" section). The genuinely-failable assertions are the
