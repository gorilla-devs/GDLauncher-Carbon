 ...
 *    own doc comment. Kept anyway as a regression check on that
 *    reconciliation pipeline itself, not dropped as dead weight.
 ...
 *    in two places written non-atomically (the `.disabled` rename on disk,
 *    then the `ModFileCache.enabled` row, in that order), and a
 *    reconciliation scan runs on every instance at startup. A restart
 ...
 *
 * The cost of that isolation, measured on this branch: a full
 * login+enrollment round trip and a **cold**
...
test.describe("state survives a restart", () => {
  ...
  test("an instance, a setting, an installed mod, and a disabled mod all survive a real app restart", async ({}, testInfo) => {
    ...
    try {
      ...
      // for more than it proves (three sabotage runs against it all stayed
      // green): every property this step can observe about an
